...
/*========================================*/
$j(document).ready(function(){  
	if (options.show_once==1 && readCookie("arevico_lb")==1) 
	{
	
	} else {
		window.setTimeout(show_lb, options.delay)
	}
});

function show_lb(){
	tb_show(options.title,'#TB_inline?height=' + options.height + '&width='+options.width +'&inlineId=hiddenContent', true);
	if (options.show_once==1){
		createCookie("arevico_lb", "1", 1);
	}
}
